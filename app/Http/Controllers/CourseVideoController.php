...
class CourseVideoController extends Controller
{
    ...
    public function store(StoreCourseVideoRequest $request, Course $course)
    {
        ...
        return redirect()->route('admin.courses.show', $course)->with('success', 'Course video created successfully');
    }
    ...
    public function update(StoreCourseVideoRequest $request, CourseVideo $courseVideo)
    {
        ...
        return redirect()->route('admin.courses.show', $courseVideo->course)->with('success', 'Course video updated successfully');
    }
    ...
    public function destroy(CourseVideo $courseVideo)
    {
        ...
        try {
            $courseVideo->delete();
            DB::commit();
            return redirect()->route('admin.courses.show', $courseVideo->course)->with('success', 'Course deleted successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('admin.courses.show', $courseVideo->course)->with('error', 'Error occurred, please try again later');
        }
    }
}
