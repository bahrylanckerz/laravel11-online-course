            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <img src="{{ Storage::url($course->thumbnail) }}" alt="image" class="rounded-2xl object-cover w-[200px] h-[150px]">
                    ...
                    <div class="flex flex-row items-center gap-x-3">
                        <a href="{{ route('admin.courses.edit', $course) }}" class="font-bold py-2 px-4 bg-indigo-700 hover:bg-indigo-800 text-white rounded-md">
                            Edit Course
                        </a>
                    </div>
                </div>
                ...
                <div class="flex flex-row justify-between items-center">
                    <div class="flex flex-col">
                        ...
                        <p class="text-slate-500 text-sm">{{ $course->videos->count() }} Total Videos</p>
                    </div>
                    <a href="{{ route('admin.course.add.video', $course) }}" class="font-bold py-2 px-4 bg-indigo-700 hover:bg-indigo-800 text-white rounded-md">
                        Add New Video
                    </a>
                </div>
                    ...
                    <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                        <div class="flex flex-row items-center gap-x-3">
                            <iframe width="560" class="rounded-2xl object-cover w-[120px] h-[90px]" height="315"
                                src="https://www.youtube-nocookie.com/embed/{{ $video->path_video }}" title="YouTube video player"
                                frameborder="0"
                        ...
                        <div class="flex flex-row items-center gap-x-3">
                            <a href="{{ route('admin.course_videos.edit', $video) }}" class="font-bold py-2 px-4 bg-indigo-700 text-white rounded-md">
                                Edit Video
                            </a>
                            <form action="{{ route('admin.course_videos.destroy', $video) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="font-bold py-2 px-4 bg-red-700 text-white rounded-md">
